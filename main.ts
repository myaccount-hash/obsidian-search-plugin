import { Plugin, TFile, FuzzySuggestModal, FuzzyMatch, setIcon, PluginSettingTab, App, Setting } from 'obsidian';
...
class SearchModal extends FuzzySuggestModal<SearchResult> {
  items: SearchResult[] = [];
  plugin: SearchPlugin;
  ...
  renderSuggestion(match: FuzzyMatch<SearchResult>, el: HTMLElement) {
    ...
    if (match.match.matches) {
      ...
      match.match.matches.forEach((m: [number, number]) => {
    ...
    if (contentMatchStart) {
      ...
      const adjustedMatches = match.match.matches
        .filter(m => m[0] >= titleText.length + 1)
        .map(m => [m[0] - titleText.length - 1 - contextStart, m[1] - titleText.length - 1 - contextStart] as [number, number])
  ...
  onChooseItem(result: SearchResult) {
